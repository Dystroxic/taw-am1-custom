...
} forEach ((KPCF_activeSpawn nearObjects GVAR(crateFillerTargetDistance)) select {!(_x getVariable [QGVAR(isCrateFiller), false])});
...
// Fill the list
{
    ...
    if (_picture isEqualTo "pictureThing") then {
        _ctrlStorage lbSetPicture [_index, QPATHTO_C(KPCF\img\icon_help.paa)];
    } else {
